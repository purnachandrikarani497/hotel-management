...
const AdminDashboard = () => {
  ...
  const [filterRole, setFilterRole] = React.useState<'all'|'user'|'owner'|'admin'>('all')
        ...
        <Card className="rounded-2xl p-0 shadow-2xl bg-gradient-to-br from-white via-purple-50 to-pink-100 border-0">
          ...
          <CardContent>
            ...
            <div className="flex items-center gap-2 mb-3 flex-wrap">
              ...
              <select className="px-3 py-2 rounded border bg-background text-sm w-full sm:w-auto" value={filterRole} onChange={e => setFilterRole(e.target.value as 'all' | 'user' | 'owner' | 'admin')}>
              ...
              <Button variant="outline" className="shrink-0" onClick={()=>{
                ...
                const data = sortRecent((users.data?.users||[]).filter(u => 
                  (filterRole === 'all' || (u.role || 'user')===filterRole) && 
              ...
              <Button variant="destructive" className="shrink-0" onClick={()=>{ 
                try { 
                  ...
                  const data = sortRecent((users.data?.users||[]).filter(u=> (filterRole === 'all' || (u.role || 'user')===filterRole) && inPeriod(usersPeriod, u.createdAt) && !map[u.id])); 
