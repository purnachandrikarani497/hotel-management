  ...
  const updatePricing = useMutation({
    ...
    onSuccess: (_res, vars) => {
      ...
      qc.setQueryData(["owner", "hotels", ownerId], (old: { hotels: Hotel[] } | undefined) => {
        ...
        return {
          hotels: old.hotels.map((h) => {
            if (h.id === vars.hotelId) {
              return {
                ...h,
                pricing: {
                  ...(h.pricing || {}),
                  normalPrice: vars.normalPrice ?? h.pricing?.normalPrice,
                  weekendPrice: vars.weekendPrice ?? h.pricing?.weekendPrice,
                  extraHourRate: vars.extraHourRate ?? h.pricing?.extraHourRate,
                  cancellationHourRate: vars.cancellationHourRate ?? h.pricing?.cancellationHourRate,
                  seasonal: vars.seasonal ?? h.pricing?.seasonal,
                  specials: vars.specials ?? h.pricing?.specials,
                },
              }
            }
            return h
                          ...
                          <tr key={h.id} className="border-t">
                              ...
                              {editing[h.id] ? (
                                <Input
                                  ...
                                  onChange={(e)=> {
                                    ...
                                    if (Number(val) <= 0 && val !== "") { toast({ title: "Price must be greater than 0", variant: "destructive" }); return; }
                            ...
                            <td className="p-3">
                              ...
                              {editing[h.id] && (
                                <div className="mt-2 flex flex-col items-start gap-2">
                                  ...
                                  />
                                  <Button size="sm" onClick={async ()=>{ const imgs = imageFiles[h.id] || []; if (!imgs.length) return; const toDataUrl = (f: File)=> new Promise<string>((resolve,reject)=>{ const r = new FileReader(); r.onload = ()=> resolve(String(r.result||'')); r.onerror = reject; r.readAsDataURL(f) }); const dataUrls = await Promise.all(imgs.map(toDataUrl)); updateImages.mutate({ id: h.id, images: dataUrls }); setUploadInfo({ type:'images', names: imgs.map(f=>f.name) }); setEditing({ ...editing, [h.id]: false }) }}>Save</Button>
                                </div>
                              )}
                            </td>
                            ...
                            <td className="p-3">
                              <div className="flex gap-2 flex-wrap">
                                <Button size="sm" variant="outline" onClick={()=>{ const next = !editing[h.id]; if (next) { setAmenitiesEdit(prev => ({ ...prev, [h.id]: prev[h.id] ?? (h.amenities||[]).join(', ') })); setLocationEdit(prev => ({ ...prev, [h.id]: prev[h.id] ?? h.location })); setPriceEdit(prev => ({ ...prev, [h.id]: prev[h.id] ?? String(h.price) })); setNameEdit(prev => ({ ...prev, [h.id]: prev[h.id] ?? h.name })); setDescriptionEdit(prev => ({ ...prev, [h.id]: prev[h.id] ?? (h.description||'') })); } setEditing({ ...editing, [h.id]: next }) }}>{editing[h.id] ? 'Stop Edit' : 'Edit'}</Button>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        if (!/^\d*$/.test(val)) return
                        if (val !== "" && Number(val) === 0) {
                          toast({ title: "Members must be greater than 0", variant: "destructive" })
                          return
                        }
                        if (val !== "" && Number(val) > 5) return
                        setRoomForm({
                          ...roomForm,
                          members: val,
                        })
                  ...
                  <div className="col-span-3 flex items-end">
                    <Button
                      onClick={async () => {
                        ...
                        if (nums.length === 0) {
                          toast({ title: "add room number", variant: "destructive" })
                          return
                        }
                        ...
                        const priceNum = Number(roomForm.price)
                        if (!String(roomForm.price).trim() || isNaN(priceNum) || priceNum <= 0) {
                          toast({ title: "Price must be greater than 0", variant: "destructive" })
                          return
                        }
                        const membersNum = Number(roomForm.members)
                        if (!String(roomForm.members).trim() || isNaN(membersNum) || membersNum <= 0) {
                          toast({ title: "Members must be greater than 0", variant: "destructive" })
                          return
                        }
                        const payloadBase = {
                          ...
                          <td className="p-3">
                            <Input
                              ...
                              onChange={(e) => {
                                ...
                                if (Number(val) <= 0 && val !== "") { toast({ title: "Members must be greater than 0", variant: "destructive" }); return; }
                    ...
                    {(hotelsQ.data?.hotels || []).map((h: Hotel)=> (
                      <tr key={`contact-${h.id}`} className="border-t">
                        ...
                        <td className="p-3"><Input placeholder="Owner Name" value={contactForm[h.id]?.ownerName ?? ''} onChange={(e)=> {
                          const val = e.target.value;
                          if (val.length > 50) {
                            toast({ title: "Maximum limit exceeded", description: "Owner Name cannot exceed 50 characters", variant: "destructive" });
                            return;
                          }
                          if (!/^[a-zA-Z\s]*$/.test(val)) {
                            toast({ title: "Invalid Owner Name", description: "Only characters allowed", variant: "destructive" });
                            return;
                          }
                        ...
                        <td className="p-3 flex gap-2"><Button size="sm" variant="outline" onClick={()=> setContactEditing({ ...contactEditing, [h.id]: !(contactEditing[h.id] || false) })}>{contactEditing[h.id] ? 'Stop Edit' : 'Edit'}</Button><Button size="sm" className="bg-gradient-to-r from-orange-500 to-pink-500 hover:from-orange-600 hover:to-pink-600 text-white font-semibold shadow-md hover:shadow-lg" onClick={()=> {
                          ...
                          setContactEditing({ ...contactEditing, [h.id]: false })
                          ...
                          <td className="p-2">
                            ...
                            <td className="p-2 flex gap-2 flex-wrap">
                              ...
                              <Button
                                size="sm"
                                onClick={() => {
                                  const normalStr = (pf.normalPrice || "").trim();
                                  if (!normalStr) { toast({ title: "Please enter the price", variant: "destructive" }); return; }
                                  const normalNum = Number(normalStr);
                                  if (isNaN(normalNum) || normalNum < 0) { toast({ title: "Price cannot be negative", variant: "destructive" }); return; }
                                  if (normalStr.length > 6 || normalNum > 999999) { toast({ title: "maximum limit exceeded", variant: "destructive" }); return; }

                                  const weekendStr = (pf.weekendPrice || "").trim();
                                  if (!weekendStr) { toast({ title: "Please enter the weekend price", variant: "destructive" }); return; }
                                  const weekendNum = Number(weekendStr);
                                  if (isNaN(weekendNum) || weekendNum < 0) { toast({ title: "Price cannot be negative", variant: "destructive" }); return; }
                                  if (weekendStr.length > 6 || weekendNum > 999999) { toast({ title: "maximum limit exceeded", variant: "destructive" }); return; }

                                  const extraStr = (pf.extraHourRate || "").trim();
                                  if (extraStr) {
                                     const extraNum = Number(extraStr);
                                     if (isNaN(extraNum) || extraNum < 0) { toast({ title: "Price cannot be negative", variant: "destructive" }); return; }
                                     if (extraStr.length > 6 || extraNum > 999999) { toast({ title: "maximum limit exceeded", variant: "destructive" }); return; }
                                  }

                                  const cancelStr = (pf.cancellationHourRate || "").trim();
                                  if (cancelStr) {
                                     const cancelNum = Number(cancelStr);
                                     if (isNaN(cancelNum) || cancelNum < 0) { toast({ title: "Price cannot be negative", variant: "destructive" }); return; }
                                     if (cancelStr.length > 6 || cancelNum > 999999) { toast({ title: "maximum limit exceeded", variant: "destructive" }); return; }
                                  }
                      ...
                      {!r.response && (
                        <div className="flex gap-3 mt-4">
                          ...
                          <Button
                            ...
                            onClick={() => {
                              const val = (reviewReply[r.id] || "").trim();
                              if (val.length > 400) {
                                toast({ title: "maximum limit exceeded", variant: "destructive" });
                                return;
                              }
                              if (!/^[a-zA-Z\s]+$/.test(val)) {
                                toast({ title: "Invalid response", description: "Only characters allowed", variant: "destructive" });
                                return;
                              }
