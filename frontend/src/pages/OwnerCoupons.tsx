...
const OwnerCoupons: React.FC = () => {
  ...
  const setCouponStatus = useMutation({ mutationFn: (p: { id:number; enabled:boolean }) => apiPost("/api/admin/coupons/"+p.id+"/status", { enabled: p.enabled }), onSuccess: (_res, vars) => { qc.invalidateQueries({ queryKey: ["owner","coupons",ownerId] }); toast({ title: vars.enabled ? "Enabled" : "Disabled", description: String(vars.id) }) }, onError: () => toast({ title: "Update failed", variant: "destructive" }) })
  const deleteCoupon = useMutation({ mutationFn: (id:number) => apiDelete(`/api/admin/coupons/${id}`), onSuccess: (_res, vars) => { qc.invalidateQueries({ queryKey: ["owner","coupons",ownerId] }); toast({ title: "Coupon deleted", description: String(vars) }) }, onError: () => toast({ title: "Delete failed", variant: "destructive" }) })
  ...
  const updateCoupon = useMutation({ mutationFn: (p: { id:number; discount?: number; usageLimit?: number; startDate?: string|null; endDate?: string|null }) => apiPost(`/api/admin/coupons/${p.id}`, p), onSuccess: (_res, vars) => { qc.invalidateQueries({ queryKey: ["owner","coupons",ownerId] }); toast({ title: "Coupon updated", description: String(vars.id) }) }, onError: () => toast({ title: "Update failed", variant: "destructive" }) })
