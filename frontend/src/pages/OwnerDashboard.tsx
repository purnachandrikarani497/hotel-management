                          ...
                          <td className="p-3">
                            <div className="flex items-center gap-2 mb-2">
                              ...
                              <select
                                ...
                                value={statusEdit[h.id] ?? h.status ?? ""}
                                onChange={(e) =>
                                  setStatusEdit({
                                    ...statusEdit,
                                    [h.id]: e.target.value,
                          ...
                          <td className="p-2">
                            ...
                            <div className="mt-4">
                              ...
                              <Input
                                placeholder="Description"
                                value={
                                  descriptionEdit[h.id] ?? h.description ?? ""
                                }
                                onChange={(e) =>
                                  setDescriptionEdit({
                                    ...descriptionEdit,
                            ...
                            <div className="flex gap-2 mt-2 items-center">
                              <input
                                ...
                                onChange={(e) =>
                                  setImageFiles({
                                    ...imageFiles,
                                    [h.id]: Array.from(e.target.files || []),
                                  })
                                }
                              />
                            </div>
                          ...
                          <td className="p-2">
                            <div className="flex gap-2">
                              <Button
                                variant="outline"
                                onClick={() => {
                                  const next = !editing[h.id]
                                  setEditing({ ...editing, [h.id]: next })
                                  toast({
                                    title: next ? "Edit enabled" : "Edit disabled",
                                    description: `Hotel #${h.id}`,
                                  })
                                }}
                              >
                                {editing[h.id] ? "Stop Edit" : "Edit"}
                              </Button>
                              <Button
                                onClick={async () => {
                                  const name = nameEdit[h.id] ?? h.name ?? ""
                                  const location =
                                    locationEdit[h.id] ?? h.location ?? ""
                                  const price = Number(
                                    priceEdit[h.id] ?? h.price ?? 0,
                                  )
                                  const description =
                                    descriptionEdit[h.id] ?? h.description ?? ""
                                  const amenities = (amenitiesEdit[h.id] || "")
                                    .split(",")
                                    .map((s) => s.trim())
                                    .filter(Boolean)
                                  const status =
                                    statusEdit[h.id] ?? h.status ?? ""
                                  if (
                                    name ||
                                    location ||
                                    price ||
                                  ...
                                  const imgFiles = (imageFiles[h.id] || []).slice(
                                    0,
                                    10,
                                  )
                                  if (imgFiles.length) {
                                    ...
                                    setUploadInfo({
                                      type: "images",
                                      names: imgFiles.map((f) => f.name),
                                    })
                                  }

                                  const docNames = Array.from(
                                    new Set(
                                      (docFiles[h.id] || []).map((f) => f.name),
                                    ),
                                  ).slice(0, 10)
                                  if (docNames.length) {
                                    updateDocs.mutate({ id: h.id, docs: docNames })
                                    setUploadInfo({
                                      type: "documents",
                                      names: docNames,
                                    })
